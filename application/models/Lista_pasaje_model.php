...
class Lista_pasaje_model extends CI_Model {
	...
	function insertar_pasaje2($data){
		$idb = $this->obtener_ultimo_id("vyp_pasajes","id_solicitud_pasaje");
		$estado = true;
		if($this->db->insert('vyp_pasajes', array('id_solicitud_pasaje'=> $idb, 'id_municipio' => $data['municipio'],'id_departamento' => $data['departamento'],  'fecha_mision' => $data['fecha_mision'], 'no_expediente' => $data['expediente'], 'empresa_visitada' => $data['empresa'], 'direccion_empresa' => $data['direccion'], 'nr' => $data['nr'], 'monto_pasaje' => $data['monto'], 'estado' => $estado )))
		{
			return "exito";
		}else{
			return "fracaso";
...
function editar_pasaje($data){
		...
		if($this->db->update('vyp_pasajes', array('id_municipio' => $data['municipio'],'id_departamento' => $data['departamento'],'fecha_mision' => $data['fecha_mision'], 'no_expediente' => $data['expediente'], 'empresa_visitada' => $data['empresa'], 'direccion_empresa' => $data['direccion'],  'monto_pasaje' => $data['monto'] ))){
			return "exito";
		}else{
			return "fracaso";
		}
