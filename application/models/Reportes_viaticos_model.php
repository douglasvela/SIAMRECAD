...
*/
    function obtenerViaticoAnualxDepto($data){
        ...
        $viaticos = $this->db->query("SELECT i.id_oficina_departamental as id_depto, o.id_departamento, depto.departamento, round(SUM(s.viatico),2) AS viatico, round(SUM(s.pasaje),2) AS pasaje, round(SUM(s.alojamiento),2) AS alojamiento, round((SUM(s.viatico)+SUM(s.pasaje)+SUM(s.alojamiento)),2) as total FROM (SELECT ev.viatico, ev.pasaje, ev.alojamiento, m.nr_empleado, m.fecha_solicitud FROM vyp_mision_oficial AS m JOIN vyp_empresa_viatico AS ev ON ev.id_mision = m.id_mision_oficial UNION SELECT 0, p.monto_pasaje, 0, p.nr, p.fecha_solicitud FROM vyp_pasajes AS p) AS s JOIN vyp_informacion_empleado AS i ON s.nr_empleado = i.nr JOIN vyp_oficinas AS o ON o.id_oficina = i.id_oficina_departamental JOIN org_departamento as depto ON depto.id_departamento=o.id_departamento WHERE year(s.fecha_solicitud)='$data' GROUP BY i.id_oficina_departamental ORDER BY SUM(s.viatico) DESC");
        return $viaticos;
    }
    function obtenerViaticoAnual($data)
    {
      $anios = implode(",", $data);//de array a cadena
        $viaticos = $this->db->query("SELECT year(s.fecha_solicitud) as anio,sum(s.viatico) as viatico,sum(s.pasaje) as pasaje,sum(s.alojamiento) as alojamiento, (sum(s.viatico) + sum(s.pasaje) + sum(s.alojamiento)) as total_anio FROM (SELECT ev.viatico, ev.pasaje, ev.alojamiento, m.nr_empleado, m.fecha_solicitud FROM vyp_mision_oficial AS m JOIN vyp_empresa_viatico AS ev ON ev.id_mision = m.id_mision_oficial UNION SELECT 0, p.monto_pasaje, 0, p.nr, p.fecha_solicitud FROM vyp_pasajes AS p) AS s WHERE YEAR(s.fecha_solicitud) IN ($anios)  group by year(s.fecha_solicitud) order by year(s.fecha_solicitud) desc");
        return $viaticos;
    }
    ...
    function obtenerViaticoMayoraMenor($data){
        ...
        $recursividad = $data['recursividad'];
            ...
            if($prueba->num_rows()>0){
              if($recursividad=="si"){
                $viaticos1= $this->db->query("SELECT s.nr_empleado, u.nombre_completo, round(SUM(s.pasaje),2) AS pasajes, round(SUM(s.viatico),2) AS viaticos, round(sum(s.alojamiento),2) as alojamientos,round((SUM(s.pasaje) + SUM(s.viatico) + sum(s.alojamiento)),2) AS total FROM (SELECT ev.viatico, ev.pasaje, ev.alojamiento, m.nr_empleado, m.fecha_solicitud,m.id_mision_oficial FROM vyp_mision_oficial AS m JOIN vyp_empresa_viatico AS ev ON ev.id_mision = m.id_mision_oficial UNION SELECT 0, p.monto_pasaje, 0, p.nr, p.fecha_solicitud,p.id_solicitud_pasaje FROM vyp_pasajes AS p) AS s INNER JOIN org_usuario as u   WHERE YEAR(s.fecha_solicitud) = '$anio' AND s.nr_empleado=u.nr AND u.id_seccion IN (SELECT DISTINCT s4.id_seccion FROM org_seccion as s1 LEFT JOIN org_seccion as s2 ON (s1.id_seccion=s2.depende or s1.id_seccion=s2.id_seccion) LEFT JOIN org_seccion as s3 ON (s2.id_seccion=s3.depende or s2.id_seccion=s3.id_seccion) LEFT JOIN org_seccion as s4 ON (s3.id_seccion=s4.depende or s3.id_seccion=s4.id_seccion) WHERE s1.depende = '$dir')  GROUP BY s.nr_empleado, u.nombre_completo ORDER BY total DESC");
              }else{
                $viaticos1= $this->db->query("SELECT s.nr_empleado, u.nombre_completo, round(SUM(s.pasaje),2) AS pasajes, round(SUM(s.viatico),2) AS viaticos, round(sum(s.alojamiento),2) as alojamientos,round((SUM(s.pasaje) + SUM(s.viatico) + sum(s.alojamiento)),2) AS total FROM (SELECT ev.viatico, ev.pasaje, ev.alojamiento, m.nr_empleado, m.fecha_solicitud,m.id_mision_oficial FROM vyp_mision_oficial AS m JOIN vyp_empresa_viatico AS ev ON ev.id_mision = m.id_mision_oficial UNION SELECT 0, p.monto_pasaje, 0, p.nr, p.fecha_solicitud,p.id_solicitud_pasaje FROM vyp_pasajes AS p) AS s INNER JOIN org_usuario as u   WHERE YEAR(s.fecha_solicitud) = '$anio' AND s.nr_empleado=u.nr AND u.id_seccion = '$dir'  GROUP BY s.nr_empleado, u.nombre_completo ORDER BY total DESC");
              }
