...
class Reportes_viaticos_model extends CI_Model {
    ...
    function viaticos_por_genero($data){
       $anio=$data['anio'];
        $viaticos= $this->db->query("SELECT i.id_genero,g.genero, round(SUM(s.viatico),2) AS viatico, round(SUM(s.pasaje),2) AS pasaje, round(SUM(s.alojamiento),2) AS alojamiento, round((SUM(s.viatico)+SUM(s.pasaje)+SUM(s.alojamiento)),2) as total FROM (SELECT m.nr_empleado,m.nombre_completo, ev.viatico, ev.pasaje, ev.alojamiento, m.fecha_solicitud,m.id_mision_oficial,ev.id_empresa_viatico,m.estado FROM vyp_mision_oficial AS m JOIN vyp_empresa_viatico AS ev ON ev.id_mision = m.id_mision_oficial UNION SELECT p.nr as nr_empleado,(SELECT concat(primer_nombre,' ',segundo_nombre,' ',primer_apellido,' ',segundo_apellido) FROM sir_empleado WHERE nr=p.nr) as nombre_completo ,('0') as viaticos, (SELECT sum(monto_pasaje) FROM vyp_pasajes WHERE id_mision=p.id_mision_pasajes/*month(fecha_mision)=p.mes_pasaje and year(fecha_mision)=p.anio_pasaje*/ AND nr=p.nr) as pasajes,('0') as alojamientos,p.fecha_solicitud_pasaje as fecha_solicitud,p.id_mision_pasajes as id_mision_oficial,(rand()),p.estado FROM vyp_mision_pasajes as p) as s JOIN sir_empleado AS i ON s.nr_empleado = i.nr JOIN org_genero as g ON g.id_genero=i.id_genero WHERE s.estado='8' AND year(s.fecha_solicitud)='$anio' GROUP BY i.id_genero ORDER BY SUM(s.viatico) DESC");
        return $viaticos;
    }
