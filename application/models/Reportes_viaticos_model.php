...
*/
    ...
    function obtenerViaticosPorOficina($data){
      ...
      $anio=$data['anio'];
        ...
        return $viaticos;
    }//hasta aqui estado de solicitud completado
    ...
    function viaticos_por_genero($data){
       $anio=$data['anio'];
        $viaticos= $this->db->query("SELECT i.id_genero,g.genero, round(SUM(s.viatico),2) AS viatico, round(SUM(s.pasaje),2) AS pasaje, round(SUM(s.alojamiento),2) AS alojamiento, round((SUM(s.viatico)+SUM(s.pasaje)+SUM(s.alojamiento)),2) as total FROM ( SELECT m.nr_empleado,m.nombre_completo, ev.viatico, ev.pasaje, ev.alojamiento, m.fecha_solicitud,m.id_mision_oficial,ev.id_empresa_viatico FROM vyp_mision_oficial AS m JOIN vyp_empresa_viatico AS ev ON ev.id_mision = m.id_mision_oficial UNION SELECT p.nr,0, 0, p.monto_pasaje, 0, p.fecha_solicitud,p.id_solicitud_pasaje,(rand()) FROM vyp_pasajes AS p) as s JOIN sir_empleado AS i ON s.nr_empleado = i.nr JOIN org_genero as g ON g.id_genero=i.id_genero WHERE year(s.fecha_solicitud)='$anio' GROUP BY i.id_genero ORDER BY SUM(s.viatico) DESC");
        return $viaticos;
    }
    function viaticos_por_mes($data,$primer_mes,$segundo_mes,$tercer_mes,$cuarto_mes,$quinto_mes,$sexto_mes){
       $anios = implode(",", $data);
       if($primer_mes=='0' && $segundo_mes=='0' && $tercer_mes=='0' && $cuarto_mes=='0' && $quinto_mes=='0' && $sexto_mes=='0'){
        $viaticos= $this->db->query("SELECT year(s.fecha) as anio,month(s.fecha) as mes,round(sum(s.viatico),2) as viatico,round(sum(s.pasaje),2) as pasaje,round(sum(s.alojamiento),2) as alojamiento, round((sum(s.viatico) + sum(s.pasaje) + sum(s.alojamiento)),2) as total_anio FROM ( SELECT m.nr_empleado,m.nombre_completo, ev.viatico, ev.pasaje, ev.alojamiento, m.fecha_solicitud,m.id_mision_oficial,ev.id_empresa_viatico,ev.fecha FROM vyp_mision_oficial AS m JOIN vyp_empresa_viatico AS ev ON ev.id_mision = m.id_mision_oficial UNION SELECT p.nr,0, 0, p.monto_pasaje, 0, p.fecha_solicitud,p.id_solicitud_pasaje,(rand()),p.fecha_solicitud FROM vyp_pasajes AS p ) as s WHERE month(s.fecha) IN (01,02,03,04,05,06,07,08,09,10,11,12) and YEAR(s.fecha) IN ($anios) group by month(s.fecha),year(s.fecha) order by month(s.fecha) desc,year(s.fecha) desc");
        }else{
          $viaticos= $this->db->query("SELECT year(s.fecha) as anio,month(s.fecha) as mes,round(sum(s.viatico),2) as viatico,round(sum(s.pasaje),2) as pasaje,round(sum(s.alojamiento),2) as alojamiento, round((sum(s.viatico) + sum(s.pasaje) + sum(s.alojamiento)),2) as total_anio FROM ( SELECT m.nr_empleado,m.nombre_completo, ev.viatico, ev.pasaje, ev.alojamiento, m.fecha_solicitud,m.id_mision_oficial,ev.id_empresa_viatico,ev.fecha FROM vyp_mision_oficial AS m JOIN vyp_empresa_viatico AS ev ON ev.id_mision = m.id_mision_oficial UNION SELECT p.nr,0, 0, p.monto_pasaje, 0, p.fecha_solicitud,p.id_solicitud_pasaje,(rand()),p.fecha_solicitud FROM vyp_pasajes AS p ) as s WHERE month(s.fecha) IN ('$primer_mes','$segundo_mes','$tercer_mes','$cuarto_mes','$quinto_mes','$sexto_mes') and YEAR(s.fecha) IN ($anios) group by month(s.fecha),year(s.fecha) order by month(s.fecha) desc,year(s.fecha) desc");
        }
        return $viaticos;
    }
