...
class Inicio_model extends CI_Model {
	...
	public function tiene_permiso_autorizar(){
		$nr = $this->session->userdata('nr_usuario_viatico');
		$query=$this->db->query("SELECT count(*) AS cantidad FROM vyp_informacion_empleado WHERE nr_jefe_inmediato|nr_jefe_departamento = '".$nr."'");

		if($query->result()[0]->cantidad > 0){
			return true;
		}else{
			return false;
		}
