        ...
        <tbody style="font-size: 11px;" id="body_poliza">
         	<?php
                ...
                if($num_poliza == "0"){
                    ...
                    $misiones = $this->db->query("SELECT c.*, UPPER(CONCAT_WS(' ', emp.primer_nombre, emp.segundo_nombre, emp.tercer_nombre)) AS nombre, UPPER(CONCAT_WS(' ', emp.primer_apellido, emp.segundo_apellido, emp.apellido_casada)) AS apellido, cf.funcional, o.id_oficina, o.nombre_oficina, lt.linea_trabajo, ei.id_seccion FROM (SELECT m2.id_mision_oficial, m2.nr_empleado, CAST(m2.fecha_mision_inicio AS CHAR) AS fecha_mision_inicio, CAST(m2.fecha_mision_fin AS CHAR) AS fecha_mision_fin, m2.fecha_solicitud, m2.no_cheque, m2.pagado_en, sum(e2.pasaje) AS pasaje, sum(e2.viatico) AS viatico, sum(e2.alojamiento) AS alojamiento, 'viatico' AS tipo_solicitud FROM vyp_mision_oficial as m2 JOIN vyp_empresa_viatico AS e2 ON m2.id_mision_oficial = e2.id_mision AND (MONTH(fecha_solicitud) = '12' AND YEAR(fecha_solicitud) = '".($anio_poliza-1)."') AND id_mision_oficial NOT IN (SELECT id_mision FROM vyp_poliza WHERE tipo_solicitud = 'viatico') ".$addsql." GROUP BY m2.id_mision_oficial UNION SELECT mp.id_mision_pasajes AS id_mision_oficial, mp.nr AS nr_empleado, mp.fechas_pasajes AS fecha_mision_inicio, mp.fechas_pasajes AS fecha_mision_fin, mp.fecha_solicitud_pasaje AS fecha_solicitud, '' AS no_cheque, 'banco' AS pagado_en, SUM(ps.monto_pasaje) AS pasaje, '0.00' AS viatico, '0.00' AS alojamiento, 'pasaje' AS tipo_solicitud FROM vyp_mision_pasajes AS mp JOIN vyp_pasajes AS ps ON mp.mes_pasaje = MONTH(ps.fecha_mision) AND mp.anio_pasaje = YEAR(ps.fecha_mision) AND ps.nr = mp.nr AND (MONTH(mp.fecha_solicitud_pasaje) = '12' AND YEAR(mp.fecha_solicitud_pasaje) = '".($anio_poliza-1)."') AND mp.id_mision_pasajes NOT IN (SELECT id_mision FROM vyp_poliza WHERE tipo_solicitud = 'pasaje')) AS c JOIN sir_empleado AS emp ON emp.nr = c.nr_empleado JOIN (SELECT MAX(id_empleado_informacion_laboral) as id_empleado_informacion_laboral, id_empleado, id_linea_trabajo, id_cargo_funcional, id_seccion FROM sir_empleado_informacion_laboral GROUP BY id_empleado ORDER BY id_empleado_informacion_laboral) AS ei ON ei.id_empleado = emp.id_empleado JOIN vyp_informacion_empleado AS ie ON ie.nr = c.nr_empleado JOIN sir_cargo_funcional AS cf ON cf.id_cargo_funcional = ei.id_cargo_funcional JOIN vyp_oficinas AS o ON o.id_oficina = ie.id_oficina_departamental JOIN org_linea_trabajo AS lt ON lt.id_linea_trabajo = ei.id_linea_trabajo ORDER BY c.fecha_solicitud, lt.linea_trabajo");
                    ...
                    $misiones = $this->db->query("SELECT c.*, UPPER(CONCAT_WS(' ', emp.primer_nombre, emp.segundo_nombre, emp.tercer_nombre)) AS nombre, UPPER(CONCAT_WS(' ', emp.primer_apellido, emp.segundo_apellido, emp.apellido_casada)) AS apellido, cf.funcional, o.id_oficina, o.nombre_oficina, lt.linea_trabajo, ei.id_seccion FROM (SELECT m2.id_mision_oficial, m2.nr_empleado, CAST(m2.fecha_mision_inicio AS CHAR) AS fecha_mision_inicio, CAST(m2.fecha_mision_fin AS CHAR) AS fecha_mision_fin, m2.fecha_solicitud, m2.no_cheque, m2.pagado_en, sum(e2.pasaje) AS pasaje, sum(e2.viatico) AS viatico, sum(e2.alojamiento) AS alojamiento, 'viatico' AS tipo_solicitud FROM vyp_mision_oficial as m2 JOIN vyp_empresa_viatico AS e2 ON m2.id_mision_oficial = e2.id_mision AND (MONTH(fecha_solicitud) <= '".$mes_poliza."' AND YEAR(fecha_solicitud) = '".$anio_poliza."') AND id_mision_oficial NOT IN (SELECT id_mision FROM vyp_poliza WHERE tipo_solicitud = 'viatico') ".$addsql." GROUP BY m2.id_mision_oficial UNION SELECT mp.id_mision_pasajes AS id_mision_oficial, mp.nr AS nr_empleado, mp.fechas_pasajes AS fecha_mision_inicio, mp.fechas_pasajes AS fecha_mision_fin, mp.fecha_solicitud_pasaje AS fecha_solicitud, '' AS no_cheque, 'banco' AS pagado_en, SUM(ps.monto_pasaje) AS pasaje, '0.00' AS viatico, '0.00' AS alojamiento, 'pasaje' AS tipo_solicitud FROM vyp_mision_pasajes AS mp JOIN vyp_pasajes AS ps ON mp.mes_pasaje = MONTH(ps.fecha_mision) AND mp.anio_pasaje = YEAR(ps.fecha_mision) AND ps.nr = mp.nr AND (MONTH(mp.fecha_solicitud_pasaje) <= '".$mes_poliza."' AND YEAR(mp.fecha_solicitud_pasaje) = '".$anio_poliza."') AND mp.id_mision_pasajes NOT IN (SELECT id_mision FROM vyp_poliza WHERE tipo_solicitud = 'pasaje')) AS c JOIN sir_empleado AS emp ON emp.nr = c.nr_empleado JOIN (SELECT MAX(id_empleado_informacion_laboral) as id_empleado_informacion_laboral, id_empleado, id_linea_trabajo, id_cargo_funcional, id_seccion FROM sir_empleado_informacion_laboral GROUP BY id_empleado ORDER BY id_empleado_informacion_laboral) AS ei ON ei.id_empleado = emp.id_empleado JOIN vyp_informacion_empleado AS ie ON ie.nr = c.nr_empleado JOIN sir_cargo_funcional AS cf ON cf.id_cargo_funcional = ei.id_cargo_funcional JOIN vyp_oficinas AS o ON o.id_oficina = ie.id_oficina_departamental JOIN org_linea_trabajo AS lt ON lt.id_linea_trabajo = ei.id_linea_trabajo ORDER BY c.fecha_solicitud, lt.linea_trabajo");
