...
$pdf->leyendas(95,18,'Arial','B',7,200,8,'GASTOS PERSONAL');
...
$pdf->leyendas(12,50,'Arial','B',9,200,8,'DETALLE DEL DESEMBOLSO DEL CREDITO PERSONAL');
...
if(empty($datos)){ $pdf->SetTextColor(255,0,0);$pdf->leyendas(65,74,'Arial','BI',12,200,8,"¡Asegurese de Guardar Los Cambios!");$pdf->SetTextColor(0);
    ...
    foreach ($datos as $value) {
        ...
        ($i > $j) ? $pdf->SetFillColor(226,239,217) : $pdf->SetFillColor(255,255,255);
            ...
            $datosCan = $DAO->mostrarAll($conexion,"select * from cancelaciones_hipotecas_clientes where cre_solic_id='$idsol'");
            foreach ($datosCan as $valueCan) {
                if($i==1){
                    $pdf->SetFillColor(255,255,255);
                    $j = 2;$i = 0;
